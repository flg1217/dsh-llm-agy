  ...
  override async *stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (agyStep !== undefined && session !== undefined) {
              ...
              if (stepType === 'tool' && toolName !== undefined) {
                ...
                if (state === 'ACTIVE') {
                  ...
                  }, ...(seq !== undefined ? [{ surfaceOp: 'append' as const, sourceEventSeqs: [seq] }] : []))
                  // 更新最近步骤的结果状态。
        ...
        const retryable = resultError !== undefined && RETRYABLE_ERROR_RE.test(resultError)
        ...
        const executionFeedbackBlocks = (): StreamChunk[] => {
          ...
          // 没有任何失败迹象(无 resultError / 无工具错 / 上下文没超限)→ 不输出
          if (resultError === undefined && failed.length === 0 && !contextExhausted) return []
          const lines: string[] = []
          if (resultError !== undefined) {
            lines.push(`**执行报错**:${resultError.slice(0, 250)}`)
          }
          if (contextExhausted) {
        ...
        // 成功:无执行错误且(正常退出或已有输出)。
        if (resultError === undefined && (code === 0 || hasOutput)) {
