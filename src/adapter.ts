  ...
  private handleToolStep(session: PersistentSession, active: ActiveTurn, step: AgyStep): void {
    ...
    if (state === 'ACTIVE') {
      ...
      if (stepIndex !== undefined && AGY_FILE_MUTATION_TOOLS.has(toolName)
        ...
        if (path !== undefined) active.editSnapshots.set(stepIndex, readFileRaw(path))
      }
    ...
    active.pendingEnrich.push(
      this.enrichToolResult(active, target, stepIndex, toolName, callName, callId, textOut, filePath, state === 'ERROR')
        .catch(() => { /* 补全失败静默:原始结果已在 */ }),
    )
  }
    ...
    if (offloaded !== undefined) {
      ...
      if (image !== undefined) {
        const ref = await commitImagePresentation(
          this.options.getAttachments?.(), image.data, image.mediaType, basename(offloaded),
        )
        if (ref !== undefined) {
          imageRef = ref
