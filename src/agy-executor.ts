...
5. 命令执行工具(pwsh / bash)的 \`description\` 参数必填:5–10 词说明这条命令做什么
   (显示在 dsh 的工具卡片上)。首次调用就带上,不要等 dsh 校验拒绝后再补。
`

/** AGY 自定义 agent 目录(用户级)。 */
function executorAgentFile(): string {
 ...
 */
export function ensureExecutorAgent(): boolean {
  const file = executorAgentFile()
  try {
    if (existsSync(file)) {
      const current = readFileSync(file, 'utf8')
      if (current === EXECUTOR_AGENT_MD) return false
    }
