 ...
 */
export function registerAgySearchTool(ctx, options) {
    const provider = new AgySearchProvider(options);
    let disposeTool;
    try {
        disposeTool = ctx.tools.register(defineTool({
            name: 'agy_web_search',
            description: 'Search the web with AGY (Antigravity CLI) deep search. Provide 1-3 queries in the required queries array; '
            ...
            async execute(args, exec) {
                ...
                // 单查询直接搜;多查询并发跑,失败即抛。
                if (queries.length === 1) {
                    return toValue(await provider.search({ query: queries[0], maxResults: MAX_RESULTS }, exec.signal));
                }
                const results = await Promise.all(queries.map((query) => provider.search({ query, maxResults: MAX_RESULTS }, exec.signal)));
...
/** 多查询结果合并:去重、轮询排列、截断。 */
function mergeResults(queries, results) {
    const seen = new Set();
    const sources = [];
    ...
    const contents = results.flatMap((result, index) => result.content === undefined || result.content.length === 0 ? [] : [`### ${queries[index]}\n\n${result.content}`]);
    return {
