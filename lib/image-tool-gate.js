 ...
 */
export function routedModelOf(exec) {
 ...
 * @returns 是否原生支持图片输入。
 */
export async function routeIsImageCapable(ctx, target) {
...
function textOnlyReadImageReason() {
...
function imageCapableAgyReason(target) {
 ...
 * @returns 注销函数。
 */
export function installImageToolGate(ctx, enabled) {
    return ctx.on('tools/pre-execute', async (exec, next) => {
        const name = exec.name;
        if (name !== 'read_image' && name !== 'read_image_agy')
            return next();
        if (!enabled())
            return next();
