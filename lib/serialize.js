...
/** 续跑兜底:仅当没有可补发内容时使用。 */
export const CONTINUE_PROMPT = '继续完成之前未完成的任务。基于当前工作区状态继续,不要重复已完成的工作,只报告新做的内容。';
/** mediaType → 临时文件扩展名。 */
const IMAGE_EXT = {
    'image/png': 'png',
    'image/jpeg': 'jpg',
...
const RUNTIME_CONSTRAINTS = [
    '运行环境约束(必须遵守):',
    '- 你的进程在多轮之间保活,回合结束时未完成的后台任务由外层自动续轮等待(结果不会丢)——但每次续轮都会增加延迟,能在单回合内完成的,不要拖到下一轮。',
    '- 长命令可能被本环境自动转为后台任务,这是正常的;转后台后应在**当前回合内**用任务状态工具(manage_task / command_status)持续轮询,直到任务真正结束(或确凿失败)再继续下一步或结束回合;报告必须基于任务的真实终态,不要用"已启动,稍后汇报"这类没有结果的收尾。',
    '- 需要结果的命令(构建/测试/截图/检查等)也可以直接前台运行并耐心等待完成。',
    '- 在 Windows 上运行 pwsh / powershell 命令时,命令开头必须加 `[Console]::OutputEncoding=[System.Text.Encoding]::UTF8;`'
        + '(否则中文输出是 GBK 字节,会被按 UTF-8 解码成乱码且无法恢复):'
        + ' `pwsh -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; <你的命令>"`。',
 ...
 */
async function serializeParts(ctx, prefix, messages) {
    const parts = [...prefix];
    const tempFiles = [];
    ...
    for (const message of messages) {
        ...
        if (text.length === 0 && notePaths.length === 0)
            continue;
...
export async function lastUserPrompt(ctx, messages) {
    const last = [...messages].reverse().find(message => message.role === 'user' && sourceOf(message).kind === 'user');
 ...
 */
export async function resumeReplayPrompt(ctx, messages, sentCount, lastSentMessageId, ownProvider = 'agy') {
    if (lastSentMessageId !== undefined && lastSentMessageId.length > 0) {
        ...
        if (at >= 0)
            return await replayFrom(ctx, messages, at + 1, ownProvider);
 ...
 */
async function replayFrom(ctx, messages, start, ownProvider) {
    let ownTurn = true;
    const selected = [];
