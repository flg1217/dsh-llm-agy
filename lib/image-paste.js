...
export function isImageCapableRoute(provider, model) {
 ...
 */
async function describeImage(ctx, block, command, proxy) {
    ...
    if (attachmentId.length > 0)
        describedImages.set(attachmentId, description);
    return description;
}
/** 已描述图片缓存:attachmentId → AGY 描述(进程内,内容寻址)。 */
const describedImages = new Map();
/** 统一描述模板:最新输入与历史消息共用同一模板(内容确定性,缓存命中)。 */
function imageContentText(description) {
 ...
 * 递归检测到它同样会拒,所以必须一并转走)。
 */
async function convertBlocks(ctx, blocks, getOptions) {
    const out = [];
 ...
 */
export async function convertPastedImages(ctx, messages, getOptions) {
    let changed = false;
    const transformed = [];
 ...
 */
export function installImageRelay(ctx, getOptions) {
