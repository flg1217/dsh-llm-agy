...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = {
      ...
      // read_image_agy 会话工具卡片(样式对齐 ui-tool ToolRow 的 read 家族行)
      tvRoot: '.dshAgy_tvRoot{display:flex;flex-direction:column}',
      tvRow: '.dshAgy_tvRow{position:relative;overflow:hidden}',
      tvLeading: '.dshAgy_tvLeading{flex-shrink:0}',
      tvTitle: '.dshAgy_tvTitle{font-weight:400}',
      tvChevron: '.dshAgy_tvChevron{color:var(--dsw-alias-label-secondary)}',
      tvSep: '.dshAgy_tvSep{flex:none;width:2px;height:2px;border-radius:1px;margin:0 8px;background:var(--dsw-alias-label-caption)}',
      tvSummary: '.dshAgy_tvSummary{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:var(--dsh-content-font-size-secondary,13px);line-height:calc(24px + var(--dsh-content-font-delta,0px));color:var(--dsw-alias-label-tertiary)}',
      tvErrorSummary: '.dshAgy_tvErrorSummary{color:var(--dsw-alias-state-error-primary)}',
      tvFileLink: '.dshAgy_tvFileLink{flex:0 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:0;padding:0;border:none;background:none;font:inherit;text-align:left;font-size:var(--dsh-content-font-size-secondary,13px);line-height:calc(24px + var(--dsh-content-font-delta,0px));color:var(--dsw-alias-label-secondary);text-decoration:underline dotted;text-decoration-color:var(--dsw-alias-label-tertiary);text-decoration-thickness:1px;text-underline-offset:3px;cursor:pointer}.dshAgy_tvFileLink:hover{color:var(--dsw-alias-label-primary);text-decoration-color:currentColor}',
    ...
    const C = {
      ...
      // read_image_agy 工具卡片(与上面 CSS 规则对象的 tv* 选择器一一对应)
      tvRoot: 'dshAgy_tvRoot', tvRow: 'dshAgy_tvRow', tvLeading: 'dshAgy_tvLeading',
    ...
    function AgyCard(props) {
      ...
      const loadModels = react.useCallback(async () => {
        ...
        setModels([])
        try {
          // 自建模型列表路由(见服务端 models-route.ts):插件卡槽位拿不到
          // 会话级 remote,llm.discoverModels 对插件卡不可达。
          const resp = await fetch('/api/llm-agy/models', { headers: { accept: 'application/json' } })
          const data = await resp.json()
          if (data?.ok === true) {
            // 保留 {id, name} 结构:面板显示 name(标签),填回/比较一律用 id——
            // 设置里的 model 是传给 AGY CLI 的 id,填标签会让 spawn 拿到非法值。
            setModels((data.models ?? []).filter((m) => m !== null && typeof m === 'object'
              && typeof m.id === 'string' && m.id.length > 0))
          }
        } catch { /* 加载失败保持现状 */ } finally {
              ...
              : models.length === 0
                ...
                : react.createElement('div', { className: C.pickerList },
                    models.map((m) => react.createElement('button', {
                      ...
                      onClick: () => pickFromList(m.id),
                    },
                      react.createElement('span', { className: C.pickerLabel }, m.name ?? m.id),
                      m.id === currentModel && react.createElement(IconCheckOutline16, { className: C.pickerCheck, size: 14 }),
                    )),
                  ),
          ),
        ),
    ...
    // ── read_image_agy 会话工具卡片 ──
    // 对齐原生 read_image 卡片行:Disclose 行(浏览图标/读取图片标题/可打开路径),
    // 展开后渲染图片画廊(presentationMeta 携带的附件引用,经 loadImage 解析为
    // 会话授权 URL)与描述文本。运行中只显示摘要行;失败显示首行错误。

    /** 行状态:运行中/中断/失败/成功(与 ToolRow 的状态语义一致)。 */
    function blockStateOf(block) {
      if (!('kind' in block)) return 'running'
      if (block.error?.code === 'interrupted') return 'stopped'
      return block.isError ? 'error' : 'ok'
    }

    /** 路径缩写:主目录 → ~,工作区前缀剥除(显示用)。 */
    function abbreviatePath(path, cwd, home) {
      let p = path
      if (typeof home === 'string' && home.length > 1 && p.startsWith(home)) p = '~' + p.slice(home.length)
      if (typeof cwd === 'string' && cwd.length > 0) {
        const root = cwd.replace(/[/\\]+$/, '')
        if (p.startsWith(root + '/') || p.startsWith(root + '\\')) p = p.slice(root.length + 1)
      }
      return p
    }
    ...
    /**
     * 卡片正文文本:render 产物是 `<path>…</path>\n<content>描述</content>` 信封,
     * 展示时剥掉信封标签只留描述(路径已由摘要行展示);非信封形状(错误文本)原样返回。
     */
    function displayText(text) {
      const m = /<content>([\s\S]*)<\/content>\s*$/.exec(text)
      const inner = (m !== null ? m[1] : text).trim()
      return inner
    }

    /** 附件引用 → 会话授权 URL 的 <img>(状态机:peek 同步缓存优先,异步兜底)。 */
    function AgyImageGallery({ image, loadImage }) {
      const [url, setUrl] = react.useState(() => {
        try {
          const u = loadImage?.peek?.(image)
          return typeof u === 'string' ? u : undefined
        } catch { return undefined }
      })
      react.useEffect(() => {
        if (loadImage === undefined || typeof url === 'string') return undefined
        let alive = true
        loadImage(image).then((u) => {
      ...
      return react.createElement('img', {
        src: url, className: C.tvImg, alt: image?.name ?? 'read_image_agy',
      })
    }
