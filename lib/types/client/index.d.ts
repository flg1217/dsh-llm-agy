  ...
  factory: (require) => {
    ...
    function AgyCard(props) {
      ...
      const loadModels = react.useCallback(async () => {
        ...
        try {
          const resp = await connection.api.llm.discoverModels({ settingsNs: 'agy', provider: 'models' })
          const found = resp?.result?.ok ? (resp.result.value?.models ?? []) : []
          setModels(found.map((m) => m.id).filter(Boolean))
      ...
      const writeModel = react.useCallback(async (value) => {
        try {
          const op = value === '' ? { path: ['model'], op: 'unset' } : { path: ['model'], op: 'set', value }
          await connection.api.settings.mutate({ ns: 'agy', ops: [op] })
      ...
      const onProxyBlur = react.useCallback(async () => {
        ...
        try {
          const op = value === '' ? { path: ['proxy'], op: 'unset' } : { path: ['proxy'], op: 'set', value }
          await connection.api.settings.mutate({ ns: 'agy', ops: [op] })
          setCurrentProxy(value)
          setSavedProxy(value)
        } catch { /* 写失败回读还原 */ loadConfig() }
      }, [currentProxy, savedProxy, connection, loadConfig])
