...
import { AGY_FILE_MUTATION_TOOLS, agyToolFilePath, enrichAgyToolResult, parseOffloadedMediaPath, readFileRaw, readImageFile, readSavedToolOutput } from './tool-preview.js';
 ...
 */
function mapImageToolCall(toolName, params) {
    if (toolName !== 'view_file')
        return undefined;
    const path = agyToolFilePath(params);
    if (path === undefined)
        return undefined;
    if (IMAGE_EXT_MEDIA[extname(path).toLowerCase()] === undefined)
        return undefined;
 ...
 */
function mapDshMcpToolCall(toolName, params) {
    if (toolName !== 'call_mcp_tool' || params === undefined)
        return undefined;
    const server = params['ServerName'] ?? params['serverName'] ?? params['server_name'];
    if (server !== DSH_MCP_SERVER_NAME)
        return undefined;
 ...
 */
function formatImageReadEnvelope(displayPath, image) {
    let scaled = '';
...
export class AgyLlmAdapter extends LlmAdapter {
    ...
    async acquire(sessionId, cwd, priorConversationId) {
        ...
        let openGate;
        this.spawnGate = new Promise((resolve) => { openGate = resolve; });
        await waitFor;
        // 门后二次查表:同一会话的两次 stream 重叠(如 abort 掉的 stream 仍停在
        // 门上、新一步已开始)会双双通过入口检查——先到者已建好会话时后者直接
        // 复用,绝不双开(双开会让先建的进程成孤儿,idle 回收找不到它)。
        const raced = this.sessions.get(sessionId);
        if (raced !== undefined && !raced.exited) {
            openGate(); // 本调用没有新进程要 init,门立即放行
            return raced;
        }
        let session;
        try {
    ...
    startSession(sessionId, cwd, priorConversationId) {
        ...
        if (useExecutor) {
            ...
            if (mcpUrl === undefined) {
                // 端点未就绪/被卸载:AGY 将以零 dsh 工具面启动("工具全 dsh 化"
                // 名存实亡)——显式告警,不要把这种状态留给现场猜。
                console.error('[llm-agy] dsh MCP 端点未就绪:dsh-executor 将以零 dsh 工具启动'
                    + '(检查 dsh-mcp 端点是否被卸载/服务未就绪)');
            }
            else {
                ensureDshMcpConfig(mcpUrl);
            }
        }
        else {
        ...
        session.initSeen = new Promise((resolve) => { session.initResolve = resolve; });
    ...
    handleToolStep(session, active, step) {
        ...
        if (state === 'ACTIVE') {
            // 参数缓存随每次事件刷新:AGY 的参数是流式生成的——首个 ACTIVE 常是
            // 空壳,后续事件才补全。缓存供 DONE 的映射与文件路径解析取用,停在
            // 空壳版本会让 diff/映射随它一起丢(undefined 不覆盖已有值)。
            if (stepIndex !== undefined && toolParams !== undefined)
                active.stepParams.set(stepIndex, toolParams);
            // 文件变更类工具:快照必须在执行开始前取。此前只在首个 ACTIVE 取,空壳
            // 事件(无路径)会让整个 diff 丢失;改为"首个能解析出路径的 ACTIVE 取
            // 一次、绝不覆盖"(更晚的事件可能已在执行中,覆盖会拿到脏快照)。
            if (stepIndex !== undefined && AGY_FILE_MUTATION_TOOLS.has(toolName)
                && !active.editSnapshots.has(stepIndex)) {
                const path = agyToolFilePath(active.stepParams.get(stepIndex));
                if (path !== undefined)
                    active.editSnapshots.set(stepIndex, readFileRaw(path));
            }
            ...
            if (stepIndex !== undefined)
                active.toolCallSeq.set(stepIndex, { seq: ev.seq, callId });
        ...
        if (entry === undefined || stepIndex === undefined)
            return;
        const seq = entry.seq;
        ...
        // 路径解析取"映射后的参数":MCP 工具的目标路径嵌在 Arguments 里
        // (如 read_image 的 {file_path}),直接用壳参数永远解析不到——读图卡片
        // 的标签与 meta.path 都靠它(2026-09-19 用户报告"预览又丢了"的根因之一)。
        const filePath = agyToolFilePath(mapped?.arguments ?? params);
        // 补全含异步 IO(读图/附件提交):登记到轮上,轮收尾前统一完成,
        // 否则回合已结束而 tool/result 尚未落地(会话事件缺一条)。
        active.pendingEnrich.push(this.enrichToolResult(active, target, stepIndex, toolName, callName, callId, textOut, filePath, state === 'ERROR')
            .catch(() => { }));
    }
    /**
     ...
     */
    async enrichToolResult(active, target, stepIndex, toolName, callName, callId, textOut, filePath, isError) {
        ...
        let imageRef;
        // AGY 的"媒体卸载"代读:MCP 返回 image 内容块(如 dsh 的 read_image)时
        // AGY 不把图片交给模型,而是落到 brain 的媒体文件,output 只剩文本信封 +
        // 一行 [Resource offloaded to ...]——实测读图卡片只有文字、没有预览。
        // 从落盘位置代读字节入库,走与 view_file 相同的信封 + image 块通道。
        const offloaded = parseOffloadedMediaPath(textOut);
        if (offloaded !== undefined) {
            const image = readImageFile(offloaded);
            if (image !== undefined) {
                const ref = await commitImagePresentation(this.options.getAttachments?.(), image.data, image.mediaType, basename(offloaded));
                if (ref !== undefined) {
                    imageRef = ref;
        ...
        if (imageRef === undefined && toolName === 'view_file') {
        ...
        // 信封的路径优先用调用参数(view_file/read_image 的目标文件),缺失时退
        // 落盘媒体路径(至少可定位)。
        const text = imageRef !== undefined
            ? formatImageReadEnvelope(filePath ?? offloaded ?? '', imageRef)
            : extra === undefined
                ? textOut
                : textOut.length > 0 ? `${textOut}\n\n${extra}` : extra;
        // 图片卡片(ui-tool image-card-model)要求**根调用**持久化展示路径:
        // `meta.path`。原生 read_image 由 tool-fs 的 presentationMeta 投影写入;
        // 适配器手写事件必须自己带上,否则卡片降级为通用行、image 块被
        // JSON.stringify 成原始 JSON 文本、预览消失(2026-09-19 用户报告)。
        // 路径用与信封相同的解析结果(content 不携带该信息,见 image-card-model)。
        const imageLabel = imageRef === undefined ? undefined : (filePath ?? offloaded ?? '');
        target.append('tool/result', {
            ...
            ...imageLabel === undefined || imageLabel === '' ? {} : { meta: { path: imageLabel } },
        }, {
            surfaceOp: 'append',
            ...(seq !== undefined ? { sourceEventSeqs: [seq] } : {}),
        });
        const last = active.translator.recentSteps.at(-1);
        if (last !== undefined && last.toolName === callName && last.status === 'running') {
    ...
    async *stream(options) {
        ...
        try {
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                // 门上等待(或 spawn)期间被 abort:不再写 prompt——spawn 门把"检查与
                // 发送"之间的窗口拉长到最长 3s,取消落在窗口内时,取消语义优先(白起
                // 的进程由 idle 回收处理)。
                if (options.signal?.aborted)
                    throw options.signal.reason ?? new Error('aborted');
