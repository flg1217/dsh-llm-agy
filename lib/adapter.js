...
import { buildPrompt, CONTINUE_PROMPT, resumeReplayPrompt } from './serialize.js';
...
async function closeWithTimeout(proc, signal, timeoutMs = 30_000) {
    if (proc.exitCode !== null || proc.signalCode !== null)
        return [proc.exitCode, proc.signalCode];
    if (signal?.aborted)
...
export class AgyLlmAdapter extends LlmAdapter {
    ...
    rememberConversation(sessionId, state) {
    ...
    async *stream(options) {
        ...
        // 见过的增量消息(--conversation 恢复)。绝不重发全量历史——token 开销
        // 巨大,且实测会被 AGY 当作新任务从头重跑;锚点缺失/压缩时只发最后一条
        // 用户输入兜底。无记忆才是首轮,发全量。
        const sessionId = options.sessionId;
        const prior = sessionId !== undefined ? this.conversations.get(sessionId) : undefined;
        ...
        try {
            // 本轮 AGY 会话 id:续跑时来自跨轮记忆(--conversation 恢复,只补发增量),
            // 首轮为空;本次调用内重试(第 2 次尝试起)复用它续跑。
            let conversationId = prior?.conversationId;
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                if (options.signal?.aborted)
                    throw options.signal.reason ?? new Error('aborted');
                // 第 2 次起:复用同一 AGY 会话续跑,而不是重头执行;中途偶发网络错误
                // 重试时不会重复已完成的工具操作与文本。续跑 prompt 不能是原任务
                // (会被当作新消息重做一遍),而是明确指示继续未完成的工作(长驻进程
                // 与后台任务规则由 RUNTIME_CONSTRAINTS 每次携带)。
                const attemptPrompt = attempt > 1 && conversationId !== undefined
                    ? CONTINUE_PROMPT
                    : prompt;
                // 有会话 id 就恢复:续跑(attempt 1,来自跨轮记忆)与本次调用内重试
                // (attempt>1)都只发新消息/续跑提示,不重头执行。
                const resumeArgs = conversationId !== undefined
                ...
                const evaluateStall = async () => {
                    if (stallChecking || stallTimedOut)
                        return;
                    stallChecking = true;
                ...
                try {
                    ...
                    for await (const line of rl) {
                        ...
                        // conversation id 统一随锚点在成功收尾时写回(失败保留旧记录)。
                        if (cid !== undefined)
                            conversationId = cid;
                        ...
                        if (agyStep !== undefined && session !== undefined) {
                            ...
                            if (stepType === 'tool' && toolName !== undefined) {
                                ...
                                if (state === 'ACTIVE') {
                                    ...
                                    // 实测毒死整个事件订阅流,子代理窗口全白)。
                                    if (stepIndex !== undefined && toolCallSeq.has(stepIndex))
                                        return;
                                    if (stepIndex !== undefined && toolParams !== undefined)
                                        stepParams.set(stepIndex, toolParams);
