...
export const IMAGE_MEDIA_BY_EXT = {
    '.png': 'image/png',
    '.jpg': 'image/jpeg',
    '.jpeg': 'image/jpeg',
    '.webp': 'image/webp',
 ...
 */
export function readImageFile(filePath, maxBytes = MAX_IMAGE_BYTES) {
...
export const AGY_FILE_MUTATION_TOOLS = new Set(['write_to_file', 'replace_file_content']);
/**
 ...
 */
export function agyToolFilePath(params) {
    if (params === undefined)
        return undefined;
...
export function readFileRaw(filePath) {
    ...
    try {
        const stat = statSync(filePath);
        if (!stat.isFile() || stat.size === 0 || stat.size > MAX_READ_BYTES)
            return undefined;
 ...
 */
export function enrichAgyToolResult(toolName, filePath, before) {
    if (toolName === 'view_file')
        return readFilePreview(filePath);
    if (AGY_FILE_MUTATION_TOOLS.has(toolName))
        return diffLineSnapshots(before, readFileRaw(filePath));
    return undefined;
}
/**
 * AGY 大输出落盘代读。
 ...
 */
export function readSavedToolOutput(conversationId, stepIndex) {
    const file = join(homedir(), '.gemini', 'antigravity-cli', 'brain', conversationId, '.system_generated', 'steps', String(stepIndex), 'output.txt');
    try {
 ...
 */
export function parseOffloadedMediaPath(text) {
