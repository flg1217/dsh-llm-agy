 ...
 */
export function latin1ToUtf8(value) {
    let hasHigh = false;
...
export function fixLatin1Deep(value) {
    if (typeof value === 'string')
        return latin1ToUtf8(value);
    if (Array.isArray(value))
        return value.map(fixLatin1Deep);
    ...
    try {
            ...
            if (typeof stepType === 'string') {
                ...
                if (stepType === 'tool') {
                    ...
                    if (info?.parameters !== undefined && typeof info.parameters === 'object') {
                        ...
                        step.toolParams = fixLatin1Deep(info.parameters);
                    }
                    if (info?.output !== undefined)
                        step.output = fixLatin1Deep(info.output);
                    if (info?.error !== undefined)
                        step.toolError = fixLatin1Deep(info.error);
                }
                out.step = step;
            }
            return out;
        ...
        if (evt.event === 'result') {
            ...
            const cid = typeof r?.conversation_id === 'string' && r.conversation_id.length > 0
                ? r.conversation_id
                : undefined;
            const withCid = (out) => (cid === undefined ? out : { ...out, conversationId: cid });
            if (r?.status === 'SUCCESS' || (typeof response === 'string' && response.length > 0)) {
                if (typeof response === 'string' && response.length > 0)
                    return withCid({ finalText: response, final: true });
...
export class AgyTranslator {
    ...
    push(line) {
        ...
        if (parsed.resultError !== undefined && this._finalText === undefined)
            this._resultError = parsed.resultError;
 ...
 */
export function agyCallId(toolName, stepIndex, attempt = 1) {
    return ToolCallId(`agy-${attempt}-${toolName}-${stepIndex ?? Math.random()}`);
}
