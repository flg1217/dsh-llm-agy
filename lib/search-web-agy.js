 ...
 */
export function registerSearchWebAgy(ctx, getProvider) {
    try {
        const offTool = ctx.tools.register(defineTool({
            name: 'search_web_agy',
            description: 'Search the web with AGY deep research: auto-search, read full sources, and return a comprehensive cited answer. '
            ...
            output: {
                ...
                render: (_args, value) => [{
                        ...
                        text: value.content !== undefined && String(value.content).length > 0
                            ...
                            : (value.sources ?? []).map((s) => s.url ?? '').join('\n'),
                    }],
            },
            ...
            async execute(args) {
                ...
                const queries = Array.isArray(raw)
                    ? raw.filter((q) => typeof q === 'string' && q.trim().length > 0)
                    : [];
                if (queries.length === 0) {
