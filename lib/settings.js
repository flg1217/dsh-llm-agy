...
export function registerAgySettings(ctx) {
    ...
    if (llm !== undefined && typeof llm.registerModelDiscovery === 'function') {
        try {
            llm
                ...
                if (action === 'models') {
                    ...
                    for (const line of text.split('\n')) {
                        ...
                        if (m !== null)
                            entries.push({ id: m[1], name: `${m[1]}  ${m[2]}` });
                    }
                    // 解析失败/无结果时回落到当前默认,避免弹窗空白
                    if (entries.length === 0)
                        entries.push({ id: section.model, name: section.model });
                    return entries;
                }
                if (action === 'test') {
            ...
            if (error?.code !== 'DUPLICATE_DISCOVERY')
                throw error;
        }
    }
    return sectionOf;
