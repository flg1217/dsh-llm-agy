 ...
 *   例如"只提取表格里的数字"、"重点说明布局结构"。省略时为通用详细描述。
 */
export async function agyReadImage(command, proxy, filePath, extra, signal) {
 ...
 */
export async function commitImagePresentation(attachments, data, mediaType, name) {
...
async function diskImagePresentation(attachments, diskPath) {
    try {
 ...
 */
async function readImageByAttachment(attachments, filePath, command, proxy, extra, signal) {
 ...
 */
export function agyReadImageAgyTool(getOptions, getAttachments = () => undefined) {
    return defineTool({
        name: 'read_image_agy',
        description: 'Read a PNG/JPEG/WebP/GIF file and describe its content via AGY/Gemini vision (a text transcription of the image). '
            + 'Use this when the current model cannot accept image input (text-only routes) — there it is the recommended image reader. '
            + 'When the current model natively accepts images, call `read_image` instead so the image itself reaches your context. '
            + 'Accepts a local file path or an attachment id (e.g. sha256:...) from a pasted image. '
            + 'Call this directly; do not delegate image reading to a subagent. '
        ...
        async execute(args, exec) {
            ...
            if (byAttachment !== undefined) {
                const image = await commitImagePresentation(getAttachments(), byAttachment.data, byAttachment.mediaType, `image.${extensionOf(byAttachment.mediaType)}`);
