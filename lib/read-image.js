...
/** AGY 读图:同步执行,返回描述文本。 */
export function agyReadImage(command, proxy, filePath) {
    ...
    if (r.error)
        throw new Error(`AGY 读图失败:${String(r.error)}`);
    if (r.status !== 0)
        throw new Error(`AGY 读图失败(exit ${r.status}):${(r.stderr ?? '').slice(0, 200)}`);
...
export function agyReadImageAgyTool(getOptions) {
    return defineTool({
        name: 'read_image_agy',
        ...
        async execute(args, exec) {
            ...
            if (!existsSync(filePath))
                throw new Error(`cannot read "${filePath}": no such file`);
